...
USER_AGENT = "AI-Assistant-MinecraftParser/1.0 (local game assistant; contact: local)"
...
def _note_from_params(params: dict[str, str]) -> str:
    ...
    if params.get("dropchance"):
        try:
            chance = float(params["dropchance"])
            if chance <= 0.05:
                notes.append("Редко")
        except ValueError:
            pass
...
def parse_drops(wikitext: str) -> list[DropEntry]:
    ...
    for line in block.split("\n"):
        ...
        count = _format_count(
            ...
            params.get("lootingquantity", ""),
        )
        note = _note_from_params(params)
        drops.append(DropEntry(item=item, count=count, note=note))
    return drops
